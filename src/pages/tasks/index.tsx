...
import { downloadFile } from '@utils/downloadContent';
...
// import { Disclaimer } froms '@/tasks/components/disclaimer';
...
export const TasksPage = (): ReactElement => {
  ...
  const handlePastToClipboard = async (content: string): void => {
    ...
    try {
      const possibleTasks: ITask[] = JSON.parse(content);
        ...
        <div className="mt-[8rem] w-full">
          <div className="font-extrabold  flex items-center">
            ...
            <div className="flex items-center ml-[80px]">
              <button
                type="button"
                onClick={(): void => handleDownloadLocalstorage()}
                className="flex items-center bg-[#212332] text-white transition-all duration-100 py-[10px] px-[16px] hover:bg-[#2F3241] shadow-xl rounded-[3px]">
                <span>
                ...
                <span className="underline text-[14px] font-semibold text-white ml-[8px]">download</span>
              </button>
